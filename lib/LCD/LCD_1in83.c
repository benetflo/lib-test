...
#if 0
    UWORD j;
    UWORD Image[LCD_1IN83_WIDTH*LCD_1IN83_HEIGHT];
    
    Color = ((Color<<8)&0xff00)|(Color>>8);
   
    for (j = 0; j < LCD_1IN83_HEIGHT*LCD_1IN83_WIDTH; j++) {
        Image[j] = Color;
    }
    
    LCD_1IN83_SetWindows(0, 0, LCD_1IN83_WIDTH, LCD_1IN83_HEIGHT);
    DEV_Digital_Write(LCD_DC_PIN, 1);
    for(j = 0; j < LCD_1IN83_HEIGHT; j++){
        DEV_SPI_Write_nByte((uint8_t *)&Image[j*LCD_1IN83_WIDTH], LCD_1IN83_WIDTH*2);
    }
#endif 
    UWORD i,j;
...
void LCD_1IN83_DisplayWindows(UWORD Xstart, UWORD Ystart, UWORD Xend, UWORD Yend, UWORD *Image)
{
    ...
    UWORD j;
    LCD_1IN83_SetWindows_Point(Xstart, Ystart, Xend , Yend);
    DEV_Digital_Write(LCD_DC_PIN, 1);;
    for (j = Ystart; j < Yend - 1; j++) {
        Addr = Xstart + j * LCD_1IN83.WIDTH;
